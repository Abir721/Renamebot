...
async def file_handler(client, message: Message):
    if message.media and getattr(message, message.media.value).file_size > MAX_FILE_SIZE:
        return await message.reply("❌ File too large! Max 2GB allowed.")

    file_name = getattr(message, message.media.value).file_name or "Unnamed"
    buttons = [
        [
...
async def cb_handler(client, cb):
    ...
    if data == "to_doc":
        ...
        bot.set_parse_mode(None)
        bot._rename_cache[user_id] = download_path
        return
    else:
...
async def rename_text(client, message: Message):
    ...
    if user_id not in bot._rename_cache:
        return

    old_path = bot._rename_cache.pop(user_id)
